...
if (!function_exists("lapsituotteiden_summa")) {
	function lapsituotteiden_summa($tilausrivi) {
		...
		// Queryyn lisä, jolla saaadaan tilausrivin alennus
		...
		// Palautetaan isätuotteen uusi hinta
		...
		if ($rivilaskuri > 0) {
			...
			while ($row = mysql_fetch_assoc($result)) {
				...
				if ($toim != "VALMISTAVARASTOON" and $toim != "SIIRTOLISTA") {
					...
					// Näytetään vain tuotteen myyntihinta
					if ($kukarow['hinnat'] == 1) {
						// Rivihinta
					...
					elseif ($kukarow['hinnat'] == 0) {
						...
						// Jos kyseessä on tuoteperheen isätuote
						// tarkistetaan että onko kyseessä summatuote.
